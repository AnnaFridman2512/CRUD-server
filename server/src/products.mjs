...
productsRouter.get("/products",  (req, res) => {//Get all products
    res.send(products);//Using loadJson to get the data from products.json file 
});

//Create product 
productsRouter.post("/products", (req, res) => {//If we want to create a new product we use post instead of get
...
productsRouter.get("/products/:id", (req, res) => {//If we want to see a single product by its id, we use :id , we save whatever is writen after products/ and saves it in id variable (can call "id" whatever instead)
...
productsRouter.put("/products/:id", (req, res) => {//If we want to update a product we use put instead of get. we use :id to save whatever is writen after products/ and saves it in id variable (can call "id" whatever instead)
    let [product]= products.filter(product => product.id == req.params.id);//"products" is an array so we filter it by id. the distructuring is to get a single object and not an array with a single object
...
productsRouter.delete("/products/:id", (req, res) => {//If we want to delete a product we use delete instead of get
